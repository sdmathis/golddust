...
static const int64 nTargetTimespan = 60 * 60;	// Golddust: 1 hour
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
   ...
   if(fNewDifficultyProtocol) {
    ...
    // Golddust: This fixes an issue where a 51% attack can change difficulty at will.
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x029209e839028448dc437a840d65f4a18488a35fbbcde728eacccafa8f7980fb");
    }
...
{
	return false;
	//commenting out this code until we properly implement GDT alerts.
	/*
    if (!CheckSignature())
        return false;
    if (!IsInEffect())
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Golddust: increase each by adding 2 to bitcoin's value.
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // Golddustd: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
